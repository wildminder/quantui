...
def test_export_tiny_model_e2e(tmp_path):
    ...
    assert g.tensor("token_embd.weight").ggml_type == GGML_Q8_0
    assert g.tensor("blk.0.attn_norm.weight").ggml_type == GGML_F32
    assert report.tensors_total == 3
    assert report.tensors_quantized == 2
...
def test_export_report_totals(tmp_path):
    ...
    assert report.qtype_histogram == {"F32": 1, "Q8_0": 2}
...
def test_cli_list_methods():
    ...
    assert "native_q8_0" in lines and "native_f32" in lines and len(lines) == 4
...
def test_cli_json_report(tmp_path):
    ...
    assert payload["qtype_histogram"] == {"F32": 1, "Q8_0": 2}
